# Configuring Assert, Check, and Ensure Reporting with BugSplat
...
**Option 1: Throttle ensures by percentage**
```ini
[Core.System]
HandleEnsurePercent=25
```

**Option 2: Disable ensure reporting entirely (keeps logging)**
```ini
[SystemSettings]
core.EnsuresAreErrors=0
```

**Option 3: Convert ensureAlways to first-failure-only**
```ini
[SystemSettings]
...
To confirm ensures are being reported to BugSplat:

1. Add a test ensure to your code:
   ```cpp
   ensure(false && "BugSplat test ensure");
   ```

2. Run your packaged build

3. Check your BugSplat dashboard for the incoming report

4. Remove the test ensure once confirmed

## Troubleshooting

**Ensures not appearing in BugSplat:**
